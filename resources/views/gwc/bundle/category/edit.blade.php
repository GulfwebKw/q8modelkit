...
<html lang="en">
	...
	<head>
		<meta charset="utf-8" />
		<title>{{__('adminMessage.websiteName')}}|{{__('adminMessage.editcategory')}}</title>
					...
					<div class="kt-content  kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">
							...
							<div class="kt-container  kt-container--fluid ">
								<div class="kt-subheader__main">
									...
									<div class="kt-subheader__breadcrumbs">
										...
										<a href="javascript:;" class="kt-subheader__breadcrumbs-link">{{__('adminMessage.editcategory')}}</a>
									</div>
								</div>
								<div class="btn-group" style="margin-top:10px;">
                                @if(auth()->guard('admin')->user()->can('bundle-category-create'))
								<a href="{{url('gwc/bundles/category')}}" class="btn btn-brand  btn-icon-sm"><i class="la la-list-ul"></i>{{__('adminMessage.listcategories')}}</a> @endif
                                </div>
							</div>
						</div>
						...
						<div class="kt-portlet__head kt-portlet__head--lg">
									<div class="kt-portlet__head-label">
										...
										<h3 class="kt-portlet__head-title">{{__('adminMessage.editcategory')}}</h3>
									</div>
									
								</div>				
										<!--begin::Form-->
					@if(auth()->guard('admin')->user()->can('bundle-category-edit'))
                    <form name="tFrm"  id="form_validation"  method="post"
                          class="kt-form" enctype="multipart/form-data" action="{{route('bundleCategory.update',$editcategory->id)}}">
                                        ...
                                        <div class="form-group row">
                                                ...
                                                <label>{{__('adminMessage.categoryname_en')}}</label>
                                                        <input type="text" class="form-control @if($errors->has('name_en')) is-invalid @endif" name="name_en"
                                                               value="{{$editcategory->name_en?$editcategory->name_en:old('name_en')}}" autocomplete="off" placeholder="{{__('adminMessage.entercategoryname_en')}}*" />
                                                ...
                                                <label>{{__('adminMessage.categoryname_ar')}}</label>
                                                        <input dir="rtl" type="text" class="form-control @if($errors->has('name_ar')) is-invalid @endif" name="name_ar"
                                                               value="{{$editcategory->name_ar?$editcategory->name_ar:old('name_ar')}}" autocomplete="off" placeholder="{{__('adminMessage.entercategoryname_ar')}}*" />
                                      ...
                                      <!--categories description -->          
                                            <div class="form-group row">
                                                ...
                                                <label>{{__('adminMessage.categorydescription_en')}}</label>
                                                        <textarea rows="3" name="details_en" class="form-control @if($errors->has('details_en')) is-invalid @endif" autocomplete="off" placeholder="{{__('adminMessage.enterseodescription_en')}}">{{$editcategory->details_en?$editcategory->details_en:old('details_en')}}</textarea>
                                                ...
                                                <label>{{__('adminMessage.categorydescription_ar')}}</label>
                                                        <textarea dir="rtl" rows="3" name="details_ar" class="form-control @if($errors->has('details_ar')) is-invalid @endif" autocomplete="off" placeholder="{{__('adminMessage.enterseodescription_ar')}}">{{$editcategory->details_ar?$editcategory->details_ar:old('details_ar')}}</textarea>
                                         ...
                                         <!-- friendly url , status , sorting -->   
											...
											<div class="kt-portlet__foot">
												<div class="kt-form__actions">
													...
													<button type="button" onClick="Javascript:window.location.href='{{url('gwc/bundles/category')}}'"  class="btn btn-secondary cancelbtn">{{__('adminMessage.cancel')}}</button>
												</div>
											</div>
										</form>
