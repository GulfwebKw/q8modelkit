...
<head>
    ...
    <meta charset="utf-8" />
    <title>{{__('adminMessage.websiteName')}}|MostSold</title>
                ...
                <div class="kt-subheader   kt-grid__item" id="kt_subheader">
                    <div class="kt-container  kt-container--fluid ">
                        <div class="kt-subheader__main">
                            <h3 class="kt-subheader__title">Most Sold Products</h3>
                            <span class="kt-subheader__separator kt-subheader__separator--v"></span>
                            <div class="kt-subheader__breadcrumbs">
                                <a href="{{url('home')}}" class="kt-subheader__breadcrumbs-home"><i class="flaticon2-shelter"></i></a>
                                <span class="kt-subheader__breadcrumbs-separator"></span>
                                <a href="javascript:;" class="kt-subheader__breadcrumbs-link">Most Sold Products</a>
                        ...
                        @if(auth()->guard('admin')->user()->can('mostsold-list'))
                            <!--begin: Datatable -->
                                <table class="table table-striped- table-bordered table-hover table-checkable " id="kt_table_1">
                                    ...
                                    <tr>
                                        <th width="10">#</th>
                                        <th width="110">{{__('adminMessage.item_code')}}</th>
                                        <th>{{__('adminMessage.title')}}</th>
                                        <th width="50">{{__('adminMessage.image')}}</th>
                                        <th width="100">Total Order</th>
                                        <th width="50">{{__('adminMessage.quantity')}}</th>
                                        <th width="50">{{__('adminMessage.retail_price')}}</th>
                                    ...
                                    @if(count($products))
                                        ...
                                        @foreach($products as $key=>$product)
                                            <tr class="search-body">
                                                ...
                                                @php
                                                    if(isset($product['product']->countdown_price) && $product['product']->countdown_price > 0) {
                                                        $price = $product['product']->countdown_price;
                                                    }
                                                    else {
                                                        $price = $product['product']->retail_price;
                                                    }
                                                @endphp
                                                <td>{!! $product['sales'] . ' Orders <br>' . $product['sales'] * $price . ' KD' !!}</td>
                                                <td>{!! $product['product']->quantity !!}</td>
                                                <td>{!! $product['product']->retail_price . ' KD' !!}</td>
                                            ...
                                            @php $p++; @endphp
                                        @endforeach
                                        <tr><td colspan="5"></td><td><h3>{{ $totalOrders }}</h3></td><td colspan="2"><h3>{{ number_format($totalPrice,3) }}</h3></td></tr>
