...
<html lang="en">
	...
	<head>
		<meta charset="utf-8" />
		<title>{{__('adminMessage.websiteName')}}|{{__('adminMessage.createapis')}}</title>
				...
				<div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor kt-wrapper" id="kt_wrapper">
					...
					<div class="kt-content  kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">
							...
							<div class="kt-container  kt-container--fluid ">
								<div class="kt-subheader__main">
									...
									<div class="kt-subheader__breadcrumbs">
										...
										<a href="javascript:;" class="kt-subheader__breadcrumbs-link">{{__('adminMessage.createapis')}}</a>
									</div>
								</div>
						...
						<div class="kt-portlet__head kt-portlet__head--lg">
									<div class="kt-portlet__head-label">
										...
										<h3 class="kt-portlet__head-title">{{__('adminMessage.createapis')}}</h3>
									</div>
					...
					@if(auth()->guard('admin')->user()->can('apis-create'))
                         ...
                         <form name="tFrm"  id="form_validation"  method="post"
                          class="kt-form" enctype="multipart/form-data" action="{{route('apis.store')}}">
                          <input type="hidden" name="_token" value="{{ csrf_token() }}">
											<div class="kt-portlet__body">
                                                ...
                                                <option value="GET" @if(old('method')) selected @endif>GET</option>
                                                <option value="POST" @if(old('method')) selected @endif>POST</option>
                                                </select>
                                                ...
                                                <input type="text" class="form-control @if($errors->has('url')) is-invalid @endif" name="url"
                                                               value="{{old('url')}}" autocomplete="off" placeholder="{{__('adminMessage.enter_url')}}*" />
                                                               @if($errors->has('url'))
                                             ...
                                             <div class="form-group row">   
                                                ...
                                                <textarea type="text" class="form-control @if($errors->has('parameters')) is-invalid @endif" name="parameters"
                                                                 autocomplete="off" placeholder="{{__('adminMessage.enter_parameters')}}">{{old('parameters')}}</textarea>
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <label>{{__('adminMessage.details')}}</label>
                                                        <textarea rows="3" id="details" name="details" class="kt-tinymce-4 form-control @if($errors->has('details')) is-invalid @endif" autocomplete="off" placeholder="{{__('adminMessage.enter_details')}}">{{old('details')}}</textarea>
