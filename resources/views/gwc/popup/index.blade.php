...
<head>
    ...
    <meta charset="utf-8" />
    <title>{{ __('adminMessage.websiteName') }}|{{ __('Popups') }}</title>
                    ...
                    <div class="kt-subheader   kt-grid__item" id="kt_subheader">
                        <div class="kt-container  kt-container--fluid ">
                            <div class="kt-subheader__main">
                                <h3 class="kt-subheader__title">{{ __('Popups') }}</h3>
                                ...
                                <div class="kt-subheader__breadcrumbs">
                                    ...
                                    <a href="javascript:;"
                                        class="kt-subheader__breadcrumbs-link">{{ __('Popups Listing') }}</a>
                            ...
                            <div class="kt-subheader__toolbar">
                                ...
                                <div class="btn-group">
                                    @if (auth()->guard('admin')->user()->can('popup-create'))
                                        <a href="{{ url('gwc/popup/create') }}" class="btn btn-brand btn-bold"><i
                            ...
                            @if (auth()->guard('admin')->user()->can('popup-list'))
                                ...
                                <table class="table table-striped- table-bordered table-hover table-checkable "
                                    ...
                                    <thead>
                                        <tr>
                                            <th width="10">#</th>
                                            <th width="20">{{ __('adminMessage.title_en') }}</th>
                                            <th width="50">{{ __('adminMessage.link') }}</th>
                                    ...
                                    <tbody>
                                        @if (count($popupLists))
                                            @php $p=1; @endphp
                                            @foreach ($popupLists as $key => $popupList)
                                                <tr class="search-body">
                                                    <td>{{ $popupLists->firstItem() + $key }}</td>

                                                    <td>{!! $popupList->title_en !!}<br>{!! $popupList->title_ar !!}</td>
                                                    <td>{!! $popupList->link !!}</td>
                                                   
                                                    <td>
                                                        @if ($popupList->image)
                                                            <img src="{!! url('uploads/popup/thumb/' . $popupList->image) !!}" width="40">
                                                        @endif
                                                    </td>
                                                    <td>
                                                        <span class="kt-switch"><label><input
                                                                    value="{{ $popupList->id }}"
                                                                    {{ !empty($popupList->is_active) ? 'checked' : '' }}
                                                    ...
                                                    <td class="kt-datatable__cell">
                                                        ...
                                                        <!--Delete modal -->
                                                        <div class="modal fade" id="kt_modal_{{ $popupList->id }}"
                                                                ...
                                                                <div class="modal-content">
                                                                    ...
                                                                    <div class="modal-footer">
                                                                        ...
                                                                        <button type="button" class="btn btn-danger"
                                                                            onClick="Javascript:window.location.href='{{ url('gwc/popup/delete/' . $popupList->id) }}'">{{ __('adminMessage.yes') }}</button>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                            ...
                                            @endforeach
                                            <tr>
                                                <td colspan="8" class="text-center">{{ $popupLists->links() }}
                                                </td>
                                            </tr>
                                        @else
                                            <tr>
