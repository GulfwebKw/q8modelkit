							...
							<div class="kt-portlet kt-portlet--mobile">
                                ...
                                @if(auth()->guard('admin')->user()->can('manufacturer-list'))
									...
									<table class="table table-striped- table-bordered table-hover table-checkable " id="kt_table_1">
                                        ...
                                        $stateProducts = App\Http\Controllers\AdminManufacturerController::countmanufactureProduct($manufacturerList->id);
                                        $profitstats = App\Http\Controllers\AdminManufacturerController::getStatistics($manufacturerList->id);
                                        @endphp
									...
									<div class="modal-content">
										...
										<div class="modal-footer">
											...
											<button type="button" class="btn btn-danger"  onClick="Javascript:window.location.href='{{url('gwc/manufacturer/delete/'.$manufacturerList->id)}}'">{{__('adminMessage.yes')}}</button>
                                                ...
                                                <img src="{!! url('uploads/users/thumb/'.$manufacturerList->image) !!}" width="40">
                                                @endif</td>
												<td>
                                                <table width="100%">
                                                <tr><td width="100">Name</td><td>{!! $manufacturerList->title_en??'--' !!}</td></tr>
                                                <tr><td>E-mail</td><td>{!! $manufacturerList->email??'--' !!}</td></tr>
                                                <tr><td>Mobile</td><td>{!! $manufacturerList->mobile??'--' !!}</td></tr>
                                                <tr><td>Username</td><td>{!! $manufacturerList->username??'--' !!}</td></tr>
                                                </table>
                                                </td>
												
												<td>
                                                <table width="100%">
                                                <tr><td width="100">Active</td><td><span class="kt-switch"><label><input value="{{$manufacturerList->id}}" {{!empty($manufacturerList->is_active)?'checked':''}} type="checkbox"  id="manufacturer" class="change_status"><span></span></label></span></td></tr>
                                                <tr><td>Home</td><td><span class="kt-switch"><label><input value="{{$manufacturerList->id}}" {{!empty($manufacturerList->is_home)?'checked':''}} type="checkbox"  id="manufacturerhome" class="change_status"><span></span></label></span></td></tr>
                                                </table>
                                                </td>
												<td>
                                                
                                                <table width="100%">
                                                <tr><td width="100">{{trans('adminMessage.orders')}}</td><td align="center"><a href="{{url('gwc/manufactureorders/'.$manufacturerList->id)}}">{{$countOrders}}</a></td></tr>
                                                <tr><td>{{trans('adminMessage.cost_price')}}</td><td align="center">{{$profitstats['costPrice']}}</td></tr>
                                                <tr><td>{{trans('adminMessage.sale_price')}}</td><td align="center">{{$profitstats['retailPrice']}}</td></tr>
                                                <tr><td>{{trans('adminMessage.profit')}}</td><td align="center">{{$profitstats['profitPrice']}}</td></tr>
                                                </table>
                                                </td>
                                                <td>
                                                <table width="100%">
                                                <tr><td width="100">{{trans('adminMessage.products')}}({{trans('adminMessage.all')}})</td><td align="center"><a href="{{url('gwc/product?manufacturer_id='.$manufacturerList->id)}}">{{$stateProducts['totalItems']}}</a></td></tr>
                                                <tr><td>{{trans('adminMessage.publish')}}</td><td align="center">{{$stateProducts['totalItems_publish']}}</td></tr>
                                                <tr><td>{{trans('adminMessage.preorder')}}</td><td align="center">{{$stateProducts['totalItems_preorder']}}</td></tr>
                                                <tr><td>{{trans('adminMessage.notpublished')}}</td><td align="center">{{$stateProducts['totalItems_unpublish']}}</td></tr>
                                                </table>
                                                </td>
