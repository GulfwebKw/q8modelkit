...
<body>
@if($orderDetails->device_type!="mobile")
<!--preloader -->
...
<div class="tt-breadcrumb">
	<div class="container">
		<ul>
            ...
            <li>@if(!empty($orderDetails->order_id)) {{$orderDetails->order_id}}@else -- @endif</li>
		</ul>
	</div>
</div>
@endif
...
<div id="tt-pageContent">
	<div class="container-indent">
		<div class="container">
			<h1 class="tt-title-subpages noborder">{{__('webMessage.orderdetails')}} <a target="_blank" href="{{url(app()->getLocale().'/order-print/'.Request()->orderid)}}" class="btn-link float-right"><i class="icon-g-55"></i>{{__('webMessage.print')}}</a></h1>
            ...
            @endif
			          ...
			          <!-- order status details -->
						...
						<div class="row" style="border:1px #CCCCCC solid;margin-top:20px;padding:20px;border-radius:5px;">
                                    ...
                                    <label for="orderid">{{__('webMessage.orderid')}} : @if($orderDetails->order_id) {{$orderDetails->order_id}}@else -- @endif</label>
									</div>
                                    </div>
                                    <div class="col-xs-12 col-md-3 col-lg-2">
                                    <div class="form-group">
                                    <label for="paymentmethod">{{__('webMessage.paymentmethod')}} : @if($orderDetails->pay_mode) {{__($orderDetails->pay_mode)}}@else -- @endif</label>
                                    </div>
                                    </div>
                                    <div class="col-xs-12 col-md-3 col-lg-2">
                                    <div class="form-group">
                                    <label for="is_paid">{{__('webMessage.payment_status')}} : @if($orderDetails->is_paid) <font color="#009900">{{strtoupper(__('webMessage.paid'))}}</font>@else <font color="#FF0000">{{strtoupper(__('webMessage.notpaid'))}}</font> @endif</label>
                                    </div>
                                    </div>
									 @php 
				...
				<table>
                    ...
                    $pixelids[]=["id"=>$productDetails->id,"quantity"=>$orderList->quantity];
            ...
            @if(!empty($TransDetails->id))					
						<div class="tt-shopcart-box">
                              ...
                              <br><br>
                                ...
                                @endif
                                 @if(!empty($TransDetails['ref']))
								<div class="col-lg-6">
							    <div class="form-group">
									<label>{{trans('webMessage.refid')}} : {{$TransDetails['ref']}} </label>
								</div>
								</div>
                                @endif
                                @if(!empty($TransDetails['auth']))
								<div class="col-lg-6">
							    <div class="form-group">
									<label>{{trans('webMessage.authcode')}} : {{$TransDetails['auth']}} </label>
								</div>
								</div>
                                @endif
...
@php
if($strLang=="en" && !empty($settingInfo->postknet_note_en)){
$postknetnote = $settingInfo->postknet_note_en;
}elseif($strLang=="ar" && !empty($settingInfo->postknet_note_ar)){
...
dataLayer.push({
  ...
  ecommerce: {
      ...
      value: {{$totalprice}},
      tax: "",
      shipping: {{$orderDetails['delivery_charges']??'0'}},
      currency: "KWD",
