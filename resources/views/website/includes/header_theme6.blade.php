...
<header id="tt-header">
			...
			<div class="tt-header-row tt-top-row">
				<div class="tt-col-left">
					<div class="tt-box-info">
						<ul>
							...
							<li><i class="icon-f-93"></i><a href="tel:{{$settingInfo->phone}}">{{$settingInfo->phone}}</a></li>
                            @endif
							<li><i class="icon-f-92"></i><a  href="javascript:;" id="trackmyorder"  title="{{__('webMessage.trackorder')}}">{{strtoupper(trans('webMessage.trackorder'))}}</a></li>
						</ul>
					</div>
				</div>
				<div class="tt-col-right ml-auto">
					<ul class="tt-social-icon">
                        ...
                        <li><a title="{{__('webMessage.whatsapp')}}" class="icon-h-11" target="_blank" href="https://api.whatsapp.com/send?phone={{$settingInfo->social_whatsapp}}&text={{__('webMessage.whatsappsharetext')}}"></a></li>
                        @endif
	...
	<nav class="panel-menu mobile-main-menu">
		<ul>
         <li><a href="{{url('/'.app()->getLocale())}}">{{trans('webMessage.home')}}</a></li>
		...
		@endif
         <li><a href="{{url(app()->getLocale().'/page/about-us')}}">{{trans('webMessage.aboutus')}}</a></li>
         <li><a href="{{url(app()->getLocale().'/contactus')}}">{{trans('webMessage.contactus')}}</a></li>
		...
		<div class="container-fluid tt-top-line">
			<div class="row">
				<div class="tt-logo-container">
					<!-- mobile logo -->
                    @if($settingInfo->logo)
					<a class="tt-logo tt-logo-alignment" href="{{url('/'.app()->getLocale())}}"><img class="tt-retina" src="{{url('uploads/logo/'.$settingInfo->logo)}}" alt="@if(app()->getLocale()=="en") {{$settingInfo->name_en}} @else {{$settingInfo->name_ar}} @endif" title="@if(app()->getLocale()=="en") {{$settingInfo->name_en}} @else {{$settingInfo->name_ar}} @endif"></a>
                    @endif
					<!-- /mobile logo -->
				</div>
			</div>
			...
			<div class="tt-header-holder">
				...
				<div class="col-8 col-lg-8">
					<div class="tt-search-type2 size-large tt-full-width tt-no-btn">
			 			<!-- tt-search -->
						<form name="topsearchform1" id="topsearchform1" method="get" action="{{url(app()->getLocale().'/search')}}" role="search">
							<i class="icon-f-85"></i>
				...
				<div class="tt-obj-options obj-move-right">
					...
					<div class="tt-desctop-parent-search tt-parent-box tt-obj-desktop-hidden">
						<div class="tt-search tt-dropdown-obj">
							...
							<div class="tt-dropdown-menu">
								<div class="container">
									  <form name="topsearchform2" id="topsearchform2" method="get" action="{{url(app()->getLocale().'/search')}}">
										<div class="tt-col">
										...
										<div class="tt-info-text">{{__('webMessage.whatareyoulookingfor')}}</div>
									...
									<div class="tt-cart-layout" id="TempOrderBoxDiv"> 
                                       ...
                                       @else 
										...
										<div class="tt-cart-content">
                                            ...
                                            @endphp
												<div class="tt-item" style="@if($t>3) display:none; @endif" >
													<a href="{{url(app()->getLocale().'/details/'.$prodDetails->id.'/'.$prodDetails->slug)}}">
														...
														<div class="tt-item-descriptions">
															...
															<div class="tt-quantity">{{$tempOrder->quantity}} X</div> <div class="tt-price">{{__('webMessage.kd')}} {{$tempOrder->unit_price}}</div>
														</div>
													</a>
											...
											<div class="tt-cart-total-row">
												...
												<div class="tt-cart-total-price">{{__('webMessage.kd')}} {{round($subTotalAmount,3)}}</div>
											</div>
					...
					<div class="tt-desctop-parent-multi tt-parent-box">
						<div class="tt-multi-obj tt-dropdown-obj">
                            @if($settingInfo->is_lang==1)
                             @if(app()->getLocale()=='ar')
							<button onclick="javascript:window.location.href='{{ url('en/' . substr(Request::getRequestUri(), 4, strlen(Request::getRequestUri()))) }}'" class="languageButton" data-tooltip="{{__('webMessage.english')}}">E</button>
                            @else
                            <button onclick="javascript:window.location.href='{{ url('ar/' . substr(Request::getRequestUri(), 4, strlen(Request::getRequestUri()))) }}'" class="languageButton" data-tooltip="{{__('webMessage.arabic')}}">ع</button>
                            @endif
                            @endif
						</div>
					</div>
                              ...
                              <nav>
								<ul>
									<li class="dropdown megamenu selected">
										<a href="{{url('/'.app()->getLocale())}}">{{strtoupper(__('webMessage.home'))}}</a>
									</li>
                                   ...
                                   @if(!empty($desktopMenusTree->childs) && count($desktopMenusTree->childs)>0)
									<li class="dropdown @if(!empty($desktopMenusTree->is_highlighted)) deals @endif @if(empty($desktopMenusTree->is_full_width)) tt-megamenu-col-01 @else megamenu @endif"><a href="{{url(app()->getLocale().'/products/'.$desktopMenusTree->id.'/'.$desktopMenusTree->friendly_url)}}">@if(app()->getLocale()=="en") {{strtoupper($desktopMenusTree->name_en)}} @else {{$desktopMenusTree->name_ar}} @endif</a>
                                    ...
                                    @else
                                    <li class="dropdown"><a href="{{url(app()->getLocale().'/products/'.$desktopMenusTree->id.'/'.$desktopMenusTree->friendly_url)}}">@if(app()->getLocale()=="en") {{strtoupper($desktopMenusTree->name_en)}} @else {{$desktopMenusTree->name_ar}} @endif</a></li>
                                    @endif
                                    @endforeach
									@endif
									<li class="dropdown deals"><a href="{{url(app()->getLocale().'/offers')}}">{{strtoupper(trans('webMessage.offers'))}}</a></li>
                                    @if(!empty($settingInfo->is_brand_active) && !empty($brandMenus) && count($brandMenus)>0)
                                    <li class="dropdown megamenu">
                                    <a href="javascript:;">{{strtoupper(trans('webMessage.brands'))}}</a>
										<div class="dropdown-menu">
                                                 <div class="row tt-col-list">
												    @foreach($brandMenus as $brandMenu)
                                                    <div class="col-sm-2">
													<a href="{{url(app()->getLocale().'/brands/'.$brandMenu->slug)}}">
                                                    @if(!empty($brandMenu->image) && $settingInfo->is_brand_image_name=='image')
