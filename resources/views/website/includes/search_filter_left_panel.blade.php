...
<div class="col-md-4 col-lg-3 col-xl-3 leftColumn aside desctop-no-sidebar">
					...
					<div class="tt-collapse open">
						...
						<div class="tt-collapse-content">
							...
							<a href="javascript:;" id="clearallsearch" class="btn-link-02" mykey="search">{{strtoupper(__('webMessage.clearall'))}}</a>
						</div>
					</div>
                    @endif
                    <!--end filter history -->
					...
					<div class="tt-collapse open">
						...
						<div class="tt-collapse-content">
							<ul class="tt-list-row">
                                ...
                                @endphp
								<li><a href="{{url(app()->getLocale().'/'.'products'.'/'.$productCategory->cid.'/'.$productCategory->friendly_url)}}">@if(app()->getLocale()=="en" && !empty($productCategory->name_en)) {{$productCategory->name_en}} @elseif(app()->getLocale()=="ar" && $productCategory->name_ar) {{$productCategory->name_ar}} @endif <span class="float-right">({{$countProductss}})</span></a></li>
                                @endforeach
							</ul>
						</div>
					</div>
					...
					<div class="tt-collapse open">
						...
						<div class="tt-collapse-content">
							<ul class="tt-list-row">
								...
								<li><a href="javascript:;" id="{{$i}}-{{$i+15}}" class="search_rangeprice" mykey="search"> {{$i}} {{__('webMessage.kd')}} — {{$i+15}} {{__('webMessage.kd')}}</a></li>
                                @endfor
							</ul>
						</div>
					</div>
						...
						<div class="tt-collapse-content">
							<ul class="tt-options-swatch options-middle">
                               @foreach($prodSizes as $prodSize)
								<li @if(!empty(Session('searchsearch_by_size')) && Session('searchsearch_by_size')==$prodSize->id) class="active" @endif><a href="javascript:;" class="search_by_size" id="{{$prodSize->id}}" mykey="search">@if(app()->getLocale()=="en" && !empty($prodSize->title_en)) {{$prodSize->title_en}} @elseif(app()->getLocale()=="ar" && $prodSize->title_ar) {{$prodSize->title_ar}} @endif</a></li>
                                @endforeach
							</ul>
						</div>
					</div>
						...
						<div class="tt-collapse-content">
							<ul class="tt-options-swatch options-middle">
                               @foreach($prodColors as $prodColor)
                                @if($prodColor->image)
                                <li style="height:30px;width:30px !important;" @if(!empty(Session('searchsearch_by_color')) && Session('searchsearch_by_color')==$prodColor->id) class="active" @endif><a href="javascript:;" class="options-colors tt-border search_by_color" id="{{$prodColor->id}}" mykey="search"><img src="{{url('uploads/color/thumb/'.$prodColor->image)}}"  width="40" height="40" style="height:30px;width:30px !important;"/></a></li>
                                @else
								<li style="height:30px;width:30px !important;" @if(!empty(Session('searchsearch_by_color')) && Session('searchsearch_by_color')==$prodColor->id) class="active" @endif><a href="javascript:;" class="options-colors tt-border search_by_color" id="{{$prodColor->id}}" mykey="search" style="background-color:{{$prodColor->color_code}};"></a></li>
                                @endif
                                @endforeach
							</ul>
						</div>
					...
					<div class="tt-collapse open">
						...
						<div class="tt-collapse-content">
                            ...
                            @foreach($mostpopularitems as $mostpopularitem)
								<a class="tt-item" href="{{url(app()->getLocale().'/directdetails/'.$mostpopularitem->id.'/'.$mostpopularitem->slug)}}">
									<div class="tt-img">
					...
					<div class="tt-collapse open">
						...
						<div class="tt-collapse-content">
							<ul class="tt-list-inline">
								...
								<li><a href="javascript:;" class="search_by_tags" mykey="search" id="{{$cattag}}">{{$cattag}}</a></li>
                                @endforeach
							</ul>
						</div>
					</div>
