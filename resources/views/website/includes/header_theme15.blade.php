...
<!--
   ...
   --> 
    ...
    @endif 
	<li class="deals"><a href="{{url(app()->getLocale().'/offers')}}">{{__('webMessage.offers')}}</a></li>
    ...
    <li>
    <a href="{{url(app()->getLocale().'/brands/'.$brandMenu->slug)}}">@if(app()->getLocale()=="en") {{$brandMenu->title_en}} @else {{$brandMenu->title_ar}} @endif</a>
    @if(!empty($brandMenu->image) && !empty($settingInfo->is_brand_image_name) && $settingInfo->is_brand_image_name=='image')
    <img src="{{url(app()->getLocale().'uploads/brand/thumb/'.$brandMenu->image)}}" style="max-width:40px;max-height:40px;float:right;margin-top:-40px;"/>
    @endif
	</li>
    @endforeach
    </ul>
    ...
    @endif
	<li><a href="{{url(app()->getLocale().'/page/'.$about_details->slug)}}">{{__('webMessage.aboutus')}}</a></li>
    <li><a href="{{url(app()->getLocale().'/contactus')}}">{{__('webMessage.contactus')}}</a></li>
    <li><a href="{{url(app()->getLocale().'/page/'.$privacy_details->slug)}}">{{__('webMessage.privacypolicy')}}</a></li>
    <li><a href="{{url(app()->getLocale().'/page/'.$terms_details->slug)}}">{{__('webMessage.termsconditions')}}</a></li>
    <li><a href="{{url(app()->getLocale().'/faq')}}">{{__('webMessage.faq')}}</a></li>								
	 </ul>
		...
		<div class="container-fluid tt-top-line">
			<div class="row">
				<div class="tt-logo-container">
					<!-- mobile logo -->
					@if($settingInfo->logo)
					<a class="tt-logo tt-logo-alignment" href="{{url(app()->getLocale().'/')}}"><img class="tt-retina" src="{{url(app()->getLocale().'uploads/logo/'.$settingInfo->logo)}}" alt="@if(app()->getLocale()=='en') {{$settingInfo->name_en}} @else {{$settingInfo->name_ar}} @endif" title="@if(app()->getLocale()=='en') {{$settingInfo->name_en}} @else {{$settingInfo->name_ar}} @endif"></a>
                    @endif
					<!-- /mobile logo -->
				</div>
			</div>
				...
				<div class="tt-col-obj tt-obj-menu obj-aligment-center">
					...
					<div class="tt-desctop-parent-menu tt-parent-box">
						<div class="tt-desctop-menu  tt-hover-03">
                         <nav>
								<ul>
									<li class="dropdown megamenu"><a href="{{url(app()->getLocale().'/')}}">{{__('webMessage.home')}}</a></li>
                                   ...
                                   @if(!empty($desktopMenusTree->childs) && count($desktopMenusTree->childs)>0)
									<li class="dropdown @if(empty($desktopMenusTree->is_full_width)) tt-megamenu-col-01 @else megamenu @endif @if(!empty($desktopMenusTree->is_highlighted)) deals @endif"><a href="{{url(app()->getLocale().'/products/'.$desktopMenusTree->id.'/'.$desktopMenusTree->friendly_url)}}">@if(app()->getLocale()=="en") {{$desktopMenusTree->name_en}} @else {{$desktopMenusTree->name_ar}} @endif</a>
                                        ...
                                        @if(!empty($desktopMenusTree->is_full_width))
													...
													<div class="row tt-col-list">
                                                      @foreach($desktopMenusTree->childs as $childCategory)
														<div class="col-sm-2">
															...
															<a href="{{url(app()->getLocale().'/products/'.$childCategory->id.'/'.$childCategory->friendly_url)}}" class="tt-title-submenu">@if(app()->getLocale()=="en") {{$childCategory->name_en}} @else {{$childCategory->name_ar}} @endif @if($childCategory->image)<img  src="{{url(app()->getLocale().'assets/images/loader.svg')}}" data-src="@if($childCategory->image) {{url(app()->getLocale().'uploads/category/thumb/'.$childCategory->image)}} @else {{url(app()->getLocale().'assets/images/custom/header-category-01.jpg')}} @endif" alt="">@endif</a>
                                                            @if(!empty($childCategory->childs) && count($childCategory->childs)>0)
                                                            
															<ul class="tt-megamenu-submenu">
                                                                @foreach($childCategory->childs as $subchildCategory)
																<li><a href="{{url(app()->getLocale().'/products/'.$subchildCategory->id.'/'.$subchildCategory->friendly_url)}}">@if(app()->getLocale()=="en") {{$subchildCategory->name_en}} @else {{$subchildCategory->name_ar}} @endif</a></li>
                                                                @endforeach
															</ul>
                                                            @endif   
														</div>
												...
												<div class="col-sm-3">
													<div class="tt-offset-7">
														<a href="@if($desktopMenusTree->offer_link) {{$desktopMenusTree->offer_link}} @else javascript:; @endif" class="tt-promo-02">@if($desktopMenusTree->offer_image)
															<img  src="{{url(app()->getLocale().'assets/images/loader.svg')}}" data-src="{{url(app()->getLocale().'uploads/category/'.$desktopMenusTree->offer_image)}}" alt="">@endif
                                            ...
                                            @foreach($desktopMenusTree->childs as $childCategory)
												<div class="col">
													<h6 class="tt-title-submenu"><a href="{{url(app()->getLocale().'/products/'.$childCategory->id.'/'.$childCategory->friendly_url)}}">@if(app()->getLocale()=="en") {{$childCategory->name_en}} @else {{$childCategory->name_ar}} @endif</a></h6>
                                                    @if(!empty($childCategory->childs) && count($childCategory->childs)>0)
													<ul class="tt-megamenu-submenu">
												    @foreach($childCategory->childs as $subchildCategory)
													<li><a href="{{url(app()->getLocale().'/products/'.$subchildCategory->id.'/'.$subchildCategory->friendly_url)}}">@if(app()->getLocale()=="en") {{$subchildCategory->name_en}} @else {{$subchildCategory->name_ar}} @endif</a></li>
                                    ...
                                    @else
                                    <li class="dropdown @if(!empty($desktopMenusTree->is_highlighted)) deals @endif"><a href="{{url(app()->getLocale().'/products/'.$desktopMenusTree->id.'/'.$desktopMenusTree->friendly_url)}}">@if(app()->getLocale()=="en") {{$desktopMenusTree->name_en}} @else {{$desktopMenusTree->name_ar}} @endif</a></li>
                                    @endif
                                    @endforeach
									@endif
                                    @if(!empty($settingInfo->is_offer_menu))
                                    <li class="dropdown deals"><a href="{{url(app()->getLocale().'/offers')}}">{{__('webMessage.offers')}}</a></li>
                                    @endif
                                    ...
                                    <a href="javascript:;">{{__('webMessage.brands')}}</a>
										<div class="dropdown-menu">
                                                 <div class="row tt-col-list">
                                                    ...
                                                    <div class="col-sm-2">
													<a href="{{url(app()->getLocale().'/brands/'.$brandMenu->slug)}}">@if(!empty($brandMenu->image) && $settingInfo->is_brand_image_name=='image')
                                                    <img src="{{url(app()->getLocale().'uploads/brand/thumb/'.$brandMenu->image)}}" style="max-width:100px;max-height:100px;"/>
                                                    @else
				...
				<div class="tt-col-obj tt-obj-options obj-move-right">
					...
					<div class="tt-desctop-parent-search tt-parent-box">
						<div class="tt-search tt-dropdown-obj">
							...
							<div class="tt-dropdown-menu">
								<div class="container">
									<form name="topsearchform1" id="topsearchform1" method="get" action="{{url(app()->getLocale().'/search')}}">
										<div class="tt-col">
									...
									<div class="tt-cart-layout" id="TempOrderBoxDiv"> 
                                       ...
                                       @else 
										...
										<div class="tt-cart-content">
                                            ...
                                            $prodImage = url(app()->getLocale().'uploads/product/thumb/'.$prodDetails->image);
                                            }else{
                                            $prodImage = url(app()->getLocale().'uploads/no-image.png');
                                            }
                                            ...
                                            $prodImage = url(app()->getLocale().'uploads/color/thumb/'.$colorImageDetails->color_image);
                                            }
                                            }
                                            ...
                                            @endphp
												<div class="tt-item" @if($t>3)style="display:none;"@endif >
													<a href="{{url(app()->getLocale().'directdetails/'.$prodDetails->id.'/'.$prodDetails->slug.'/'.$strLang)}}">
														<div class="tt-item-img">
															<img src="{{url(app()->getLocale().'assets/images/loader.svg')}}" data-src="{{$prodImage}}" alt="@if(app()->getLocale()=='en') {{$prodDetails->title_en}} @else {{$prodDetails->title_ar}} @endif">
														</div>
											...
											@if($t>3)
											<div class="tt-item" align="center"><a href="{{url(app()->getLocale().'/cart')}}">{{trans('webMessage.viewall')}}(+{{$t-4}})</a></div>
											@endif
											</div>
											<div class="tt-cart-total-row">
												...
												@endphp
												@if ( $bundleDiscount > 0 )
													<div class="tt-cart-total-title">{{ __('webMessage.bundles.BundleDiscount') }}:
													</div>
													<div class="tt-cart-total-price" style="color: #FF0000;"> {{ round($bundleDiscount, 3) }}
														{{ __('webMessage.kd') }}</div>
											</div><div class="tt-cart-total-row" style="margin-top: 0px;padding-top: 10px;border-top: 0px;">
												@endif
											...
											<div class="tt-cart-btn">
												<div class="tt-item">
													<a href="{{url(app()->getLocale().'/checkout')}}" class="btn">{{__('webMessage.checkout')}}</a>
												</div>
												<div class="tt-item">
													<a href="{{url(app()->getLocale().'/cart')}}" class="btn-link-02 tt-hidden-mobile">{{__('webMessage.viewcart')}}</a>
													<a href="{{url(app()->getLocale().'/cart')}}" class="btn btn-border tt-hidden-desctope">{{__('webMessage.viewcart')}}</a>
												</div>
											</div>
										</div>
                                        @endif
							...
							<div class="tt-dropdown-menu">
								...
								<div class="tt-dropdown-inner">
									<ul>
                                        ...
                                        <li><a href="{{url(app()->getLocale().'/account')}}"><i class="icon-f-94"></i>{{__('webMessage.dashboard')}}</a></li>
									    <li><a href="{{url(app()->getLocale().'/changepass')}}"><i class="icon-g-40"></i>{{__('webMessage.changepassword')}}</a></li>
									    <li><a href="{{url(app()->getLocale().'/editprofile')}}"><i class="icon-01"></i>{{__('webMessage.editprofile')}}</a></li>
                                        <li><a href="{{url(app()->getLocale().'/myorders')}}"><i class="icon-f-68"></i>{{__('webMessage.myorders')}}</a></li>
                                        <li><a href="{{url(app()->getLocale().'/wishlist')}}"><i class="icon-n-072"></i>{{__('webMessage.wishlists')}}</a></li>
                                        ...
                                        <form id="logout-forms" action="{{ url(app()->getLocale().'/logout') }}" method="POST" style="display: none;">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        </form>
                                        @else
									    <li><a href="{{url(app()->getLocale().'/login')}}"><i class="icon-f-76"></i>{{__('webMessage.signin')}}</a></li>
									    <li><a href="{{url(app()->getLocale().'/register')}}"><i class="icon-f-94"></i>{{__('webMessage.signup')}}</a></li>
                                        @endif
									</ul>
								</div>
							</div>
					...
					<div class="tt-desctop-parent-multi tt-parent-box">
						<div class="tt-multi-obj tt-dropdown-obj">
							...
							@if(app()->getLocale()=='ar')
							<button onclick="javascript:window.location.href='{{route(Route::currentRouteName(), ['locale' => 'en'])  }}'" class="languageButton" data-tooltip="{{__('webMessage.english')}}">{{trans('webMessage.english')}}</button>
                            @else
						
                            <button onclick="javascript:window.location.href='{{route(Route::currentRouteName(), ['locale' => 'ar'])  }}'" class="languageButton" data-tooltip="{{__('webMessage.arabic')}}">{{trans('webMessage.arabic')}}</button>
                            @endif
		...
		<div class="container">
			<div class="tt-header-row ">
				<!--<div class="tt-stuck-parent-logo">@if($settingInfo->logo)
					<a class="tt-logo tt-logo-alignment" href="{{url(app()->getLocale().'/')}}"><img class="stuck_logo" src="{{url(app()->getLocale().'uploads/logo/'.$settingInfo->logo)}}" alt="@if(app()->getLocale()=="en") {{$settingInfo->name_en}} @else {{$settingInfo->name_ar}} @endif" title="@if(app()->getLocale()=="en") {{$settingInfo->name_en}} @else {{$settingInfo->name_ar}} @endif"></a>
                    @endif
                    </div>-->
