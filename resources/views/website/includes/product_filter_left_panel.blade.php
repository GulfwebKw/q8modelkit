...
<div class="col-md-4 col-lg-3 col-xl-3 leftColumn aside desctop-no-sidebar ">
					...
					<div class="tt-collapse open">
						...
						<div class="tt-collapse-content">
							...
							<a href="javascript:;" id="clearallfilter" class="btn-link-02" mykey="{{Request()->catid}}">{{strtoupper(__('webMessage.clearall'))}}</a>
						</div>
					</div>
                    @endif
                    <!--end filter history -->
					...
					<div class="tt-collapse open">
						...
						<div class="tt-collapse-content">
							<ul class="tt-list-row">
								...
								<li><a href="javascript:;" id="{{$i}}-{{$i+15}}" class="rangeprice" mykey="{{Request()->catid}}"> {{$i}} {{__('webMessage.kd')}} —  {{$i+15}} {{__('webMessage.kd')}}</a></li>
                                @endfor
							</ul>
						</div>
					</div>
						...
						<div class="tt-collapse-content">
							<ul class="tt-options-swatch options-middle">
                               @foreach($prodColors as $prodColor)
                                @if($prodColor->image)
                                <li style="height:30px;width:30px !important;" @if(!empty(session(Request()->catid.'filter_by_color')) && session(Request()->catid.'filter_by_color')==$prodColor->id) class="active" @endif><a href="javascript:;" class="options-colors tt-border filter_by_color" id="{{$prodColor->id}}" mykey="{{Request()->catid}}"><img src="{{url('uploads/color/thumb/'.$prodColor->image)}}"  width="40" height="40"/></a></li>
                                @else
								<li style="height:30px;width:30px !important;" @if(!empty(session(Request()->catid.'filter_by_color')) && session(Request()->catid.'filter_by_color')==$prodColor->id) class="active" @endif><a href="javascript:;" class="options-colors tt-border filter_by_color" mykey="{{Request()->catid}}" id="{{$prodColor->id}}" style="background-color:{{$prodColor->color_code}};"></a></li>
                                @endif
                                @endforeach
							</ul>
						</div>
					...
					<div class="tt-collapse open">
						...
						<div class="tt-collapse-content">
							<ul class="tt-list-inline">
								...
								<li><a href="javascript:;" class="filter_by_tags" id="{{$cattag}}" mykey="{{Request()->catid}}">{{$cattag}}</a></li>
                                @endforeach
							</ul>
						</div>
					</div>
