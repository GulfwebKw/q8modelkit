...
<head>
<meta charset="utf-8">
<title>@if(app()->getLocale()=="en") {{$settingInfo->name_en}} @else {{$settingInfo->name_ar}} @endif | {{__('webMessage.offers')}}</title>
...
<div class="tt-breadcrumb">
	<div class="container">
		<ul>
			...
			<li><a href="javascript:;">{{__('webMessage.offers')}}</a></li>
		</ul>
	</div>
</div>
				...
				<div class="col-md-12">
					<div class="content-indent">
						<div class="tt-filters-options desctop-no-sidebar">
							<h1 class="tt-title">
								{{__('webMessage.offers')}} <span class="tt-title-total">({{count($offerProductLists)}})</span>
							</h1>
							...
							<div class="tt-sort">
					<select name="offer_sort_by" id="offer_sort_by" mykey="offer">
                    <option value="">{{__('webMessage.latestitems')}}</option>
                    <option value="popular" @if(Session('offeroffer_sort_by')=="popular") selected @endif>{{__('webMessage.mostpopular')}}</option>
					<option value="max-price" @if(Session('offeroffer_sort_by')=="max-price") selected @endif>{{__('webMessage.max_price')}}</option>
					<option value="min-price" @if(Session('offeroffer_sort_by')=="min-price") selected @endif>{{__('webMessage.min_price')}}</option>
					<option value="a-z" @if(Session('offeroffer_sort_by')=="a-z") selected @endif>{{__('webMessage.atoz')}}</option>
                    <option value="z-a" @if(Session('offeroffer_sort_by')=="z-a") selected @endif>{{__('webMessage.ztoa')}}</option>
					</select>
					<select name="offer_per_page" id="offer_per_page" mykey="offer">
					<option value="12" @if(Session('offeroffer_per_page')=="12") selected @endif>{{__('webMessage.show')}}</option>
					<option value="24" @if(Session('offeroffer_per_page')=="24") selected @endif>24</option>
					<option value="48" @if(Session('offeroffer_per_page')=="48") selected @endif>48</option>
					<option value="96" @if(Session('offeroffer_per_page')=="96") selected @endif>96</option>
					</select>
							</div>
                        ...
                        <span id="responseMsg-{{$productList->id}}"></span> 
							...
							<h2 class="tt-title"><a href="{{url(app()->getLocale().'/directdetails/'.$productList->id.'/'.$productList->slug)}}">{!!Common::getLangString($productList->title_en,$productList->title_ar)!!}{!!Common::getLangStringExtra($productList->extra_title_en,$productList->extra_title_ar)!!}</a></h2>
							...
							<div class="tt-product-inside-hover">
								...
								<div class="tt-row-btn">
                                    ...
                                    <a href="{{url(app()->getLocale().'/directdetails/'.$productList->id.'/'.$productList->slug)}}" class="tt-btn-addtocart thumbprod-button-bg" id="{{$productList->id}}">{{__('webMessage.details')}}</a>
                                    @else
                                    @if(!empty($isStock))
