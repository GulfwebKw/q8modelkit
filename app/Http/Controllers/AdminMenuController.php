...
class AdminMenuController extends Controller
{
	...
	public function putmesession(Request $request){
	 if(!empty($request->keyname) && !empty($request->id)){
	 $menusDetails = Menus::where('id',$request->id)->first(); 
	 if(!empty($menusDetails->id)){
	 Session::put('parentmenu',$menusDetails->id);
	 }else{
	 Session::forget('parentmenu');
	 }
	 Session::put('childmenu',$request->id);
	 }
    ...
    public function index(Request $request)
    {
        ...
        return view('gwc.menus.adminMenus',['menusLists' => $menusLists]);
    }
    ...
    public function adminMenusForm($id="",Request $request){
        ...
        return view('gwc.menus.adminMenusForm',['menusDetails'=>$menusDetails,'menuDropDownList'=>$menuDropDownList]);
    }
