...
class AdminVendorController extends Controller
{
    ...
    {
	 return view('gwc.vendor.home');
	}
	
	//view forgot password screen
	public function forgotview(){
	return view('gwc.vendor.forgot');
	}
	...
	$this->validate($request, [
      ...
      try{
        ...
        if (Auth::guard('admin')->attempt(['username' => $request->login_username, 'password' => $request->login_password,'is_active'=>1,'userType'=>'vendor'],$remember)) {
		    //save logs
			$key_name   = "login";
			$key_id     = Auth::guard('admin')->user()->id;
			$message    = Auth::guard('admin')->user()->name."(".Auth::guard('admin')->user()->userType.") is logged in to Vendor Panel.";
	...
	return redirect('/vendor/forgot')
	 ...
	 $appendMessage = "<b><a href='".url('vendor/forgot/'.$token)."'>".trans('adminMessage.passwordresetlink')."</b>";
	 $data = [
	 ...
	 'subject' =>'Vendor Forgot Password Reset Link',
	...
	//field validation
	  $validator = Validator::make($request->all(),[
	    ...
	    if ($validator->fails()) {
            return redirect('/vendor/forgot/'.$token)
                        ->withErrors($validator)
                        ->withInput();
        }
	...
	return redirect('/vendor/forgot/'.$token)
	 ...
	 'subject' =>'Vendor Password Successfully Reset',
	 'email_from' =>$settingInfo->from_email,
	 'email_from_name' =>$settingInfo->from_name
	 ];
     Mail::to($request->email)->send(new SendGrid($data));	 
