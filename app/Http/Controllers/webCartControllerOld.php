...
class webCartController extends Controller
{
		...
		if (!empty($request->keyname) && strlen($request->keyname) >= 2) {
			...
			if (!empty($products) && count($products) > 0) {
				...
				foreach ($products as $product) {
					...
					$result .= '<li><a href="' . url('details/' . $product->id . '/' . $product->slug) . '"><div class="thumbnail"><img src="' . $imgurl . '" alt=""></div><div class="tt-description"><div class="tt-title">' . $title . '</div><div class="tt-price"><span class="new-price ' . $cl . '">' . $price . ' ' . trans('webMessage.kd') . '</span>' . $oldprice . '</div></div></a></li>';
				}
				$result .= '</ul>';
				$message = $result;
			} else {
	...
	public function saveconfirm(Request $request)
	{
		...
		if ($request->payment_method == "COD" || $request->payment_method == "POSTKNET") {
			...
			if ($payprocessDetails['status'] == 1) {
				$payURL = $payprocessDetails['payurl'];
	...
	*/
	...
	public function csreturnurl()
	{
		...
		return redirect('/order-details/' . $sid);
	}
	...
	public function customizeProduct(Request $request, $id)
	{
		...
		if ($product) {
			if ($product->customizable == 0) {
				return redirect('/details/' . $id . '/' . $product->slug);
			}
