...
class webCartController extends Controller
{
	...
	public static function ajax_reload_temp_order_box()
	{
		...
		if (empty($tempOrders) || count($tempOrders) == 0) {
			...
			if ($t >= 4) {
				$temp .= '<div class="tt-item" align="center"><a href="' . url(app()->getLocale() . '/cart') . '" >' . trans('webMessage.viewall') . '(+' . ($t - 4) . ')</a></div>';
			}
			$temp .= '</div>';
			...
			if ($bundleDiscount > 0) {
				...
				$temp .= '<div class="tt-cart-total-price" style="color: #FF0000;" id="BundleDiscountJSGet">-' . $bundleDiscount  . trans('webMessage.kd') . '</div>';
				$temp .= '</div>';
			...
			$temp .= '<div class="tt-cart-total-price" id="subtotalJSget">' . round($subTotalAmount, 3) . '' . trans('webMessage.kd') . '</div>';
			$temp .= '</div>';
	...
	public function viewProductDetails(Request $request)
	{
		...
		$productoptions = ProductOptionsCustomChosen::with('mainoption')->where('product_id', $id)->orderBy('custom_option_id', 'ASC')->get();
		//brand 
		$brandDetails = [];
		if (!empty($productDetails->brand_id)) {
		...
		if ($settingInfo->theme == 9)
			...
			return view('website.details.details17', compact("productDetails", "prodGalleries", "availableQty", "ReviewsLists", "productoptions", "brandDetails", "relatedProducts", "tagsDetails"));
		else
	...
	public static function ajax_product_quick_search(Request $request)
	{
		...
		if (!empty($request->keyname) && strlen($request->keyname) >= 2) {
    		$search = strtolower($request->keyname);
    			if ($search == 'out of stock') {
    				$products = Product::where('is_active', '!=', 0)->where('quantity',  0);
    			} else {
			...
			if (!empty($products) && count($products) > 0) {
				...
				foreach ($products as $product) {
					...
					if (!empty($product->old_price)) {
						$oldprice = '<span class="old-price price_black">' . $product->old_price . ' ' . trans('webMessage.kd') . '</span>';
						$cl      = 'price_red';
					...
					if (!empty($product->countdown_datetime) && strtotime($product->countdown_datetime) > strtotime(date('Y-m-d'))) {
						$oldprice  = '<span class="old-price price_black">' . round($product->retail_price, 3) . ' ' . trans('webMessage.kd') . '</span>';
					...
					$result .= '<li><a href="' . url(app()->getLocale() . '/details/' . $product->id . '/' . $product->slug) . '"><div class="thumbnail"><img src="' . $imgurl . '" alt=""></div><div class="tt-description"><div class="tt-title">' . $title . '</div><div class="tt-price"><span class="new-price ' . $cl . '">' . $price . ' ' . trans('webMessage.kd') . '</span>' . $oldprice . '</div></div></a></li>';
				}
				$result .= '</ul>';
				$message = $result;
			} else {
	...
	public function saveconfirm(Request $request)
	{
		...
		if (!empty($settingInfo->min_order_amount) && !empty($totalAmtchk) && $settingInfo->min_order_amount >  $totalAmtchk) {
			...
			return redirect(app()->getLocale() . '/cart')->with('session_msg', trans('webMessage.minimumordermessage') . ' ' . number_format($settingInfo->min_order_amount, 3) . ' ' . trans('webMessage.kd'));
		}
		...
		$deliveryCharge = self::get_delivery_charge($request->area ?? null);
		...
		foreach ($tempOrders as $tempOrder) {
			...
			$ordertxt_child .= '<tr>
						...
						<td>' . trans('webMessage.kd') . ' ' . $unitprice . '</td>
						<td align="center">' . $tempOrder->quantity . '</td>
						<td>' . trans('webMessage.kd') . ' ' . $subtotalprice . '</td>
						</tr>';
			$orders = new Orders;
		...
		$orderDetailsTxt .= '<tr><td colspan="4" align="right"><b>' . trans('webMessage.subtotal') . '&nbsp;:&nbsp;&nbsp;</b></td><td>' . trans('webMessage.kd') . '' . $totalprice . '</td></tr>';
		//bundle discount
		if ($bundleDiscount > 0) {
			$orderDetailsTxt .= '<tr><td colspan="4" align="right">' . trans('webMessage.bundles.BundleDiscount') . '&nbsp;:&nbsp;&nbsp;</td><td><font color="#FF0000">-' . trans('webMessage.kd') . ' ' . $bundleDiscount . '</font></td></tr>';
			$totalprice = $totalprice - $bundleDiscount;
		}
		//seller discount
		if (!empty($orderDetails->seller_discount)) {
			$orderDetailsTxt .= '<tr><td colspan="4" align="right">' . trans('webMessage.seller_discount') . '&nbsp;:&nbsp;&nbsp;</td><td><font color="#FF0000">-' . trans('webMessage.kd') . ' ' . $orderDetails->seller_discount . '</font></td></tr>';
		...
		if (!empty($orderDetails->coupon_code) && empty($orderDetails->coupon_free)) {
			$orderDetailsTxt .= '<tr><td colspan="4" align="right">' . trans('webMessage.coupon_discount') . '&nbsp;:&nbsp;&nbsp;</td><td><font color="#FF0000">-' . trans('webMessage.kd') . ' ' . $orderDetails->coupon_amount . '</font></td></tr>';
		...
		if (!empty($settingInfo->is_free_delivery) && $totalprice >= $settingInfo->free_delivery_amount) {
			...
			if (!empty($orderDetails->delivery_charges) && empty($orderDetails->coupon_free)) {
				...
				$orderDetailsTxt .= '<tr><td colspan="4" align="right">' . trans('webMessage.delivery_charge') . '&nbsp;:&nbsp;&nbsp;</td><td>' . trans('webMessage.kd') . ' ' . $deliveryCharge . '</td></tr>';
				$totalprice = $totalprice + $deliveryCharge;
			}
		}
		$orderDetailsTxt .= '<tr><td colspan="4" align="right"><b>' . trans('webMessage.grandtotal') . '</b>&nbsp;:&nbsp;&nbsp;</td><td>' . trans('webMessage.kd') . ' ' . $totalprice . '</td></tr>';
			...
			if (!empty($ResTranData)) {
				...
				if (!empty($decryptedDatas) && count($decryptedDatas) > 0) {
					...
					if (!empty($res['status']) && $res['status'] == 1) {
						return redirect(app()->getLocale() . '/order-details/' . md5($decryptedDatas['trackid']))->with('session_msg', $knetMessage);
					} else if (!empty($res['status']) && $res['status'] == 2) {
						return redirect(app()->getLocale() . '/order-details/' . md5($decryptedDatas['trackid']))->with('session_msg_error', $knetMessage);
					} else {
	...
	public function orderprint(Request $request)
	{
		...
		foreach ($tempOrders as $tempOrder) {
			...
			$ordertxt_child .= '<tr>
                    <td><a href="' . url(app()->getLocale() . '/details/' . $productDetails->id . '/' . $productDetails->slug) . '"><img src="' . url('uploads/product/thumb/' . $productDetails['image']) . '" alt="' . $title . '" width="50"></a>
					</td>
	...
	public static function ajax_apply_coupon_to_cart(Request $request)
	{
		...
		if (!empty($coupon->id) && ($total < $coupon->price_start || $total > $coupon->price_end)) {
			return ["status" => 200, "message" => '<div class="alert-danger ">' . trans('webMessage.coupon_can_be_apply_for_price_range') . trans('webMessage.kd') . ' ' . $coupon->price_start . ' - ' . trans('webMessage.kd') . ' ' . $coupon->price_end . '---' . $total . '</div>'];
		}
		...
		if (!empty($coupon->id) && $coupon->coupon_type == "amt") {
			...
			$discountAmttxt = 	trans('webMessage.kd') . ' ' . $discountAmt;
		} else {
		...
		$deliverytxt .= '<tr><th>' . strtoupper(trans('webMessage.subtotal')) . '</th>
                   <td>' . trans('webMessage.kd') . ' <span class="total_result">' . $totalprice . '</span></td>
                   </tr>';
		...
		if (!empty($deliveryCharge) && empty($coupon->is_free)) {
			$deliverytxt .= '<tr><th>' . strtoupper(trans('webMessage.delivery_charge')) . '</th>
				   <td>' . trans('webMessage.kd') . ' ' . $deliveryCharge . '</td>
				   </tr>';
			$totalprice = $totalprice + $deliveryCharge;
		}
		$deliverytxt .= '</tbody>';
	...
	public static function ajax_apply_seller_discount_to_cart(Request $request)
	{
		...
		if (!empty($request->seller_discount)) {
			$deliverytxt .= '<tr><th>' . strtoupper(trans('webMessage.seller_discount')) . '</th><td><font color="#FF0000">-' . trans('webMessage.kd') . '' . (float)$request->seller_discount . '</font></td>
									</tr>';
	...
	public static function ajax_get_area_delivery(Request $request)
	{
		...
		if (!empty($request->areaid)) {
			...
			if ($request->bundleDiscount > 0) {
				$deliverytxt .= '<tr><th>' . strtoupper(trans('webMessage.bundles.BundleDiscount')) . '</th><td><font color="#FF0000">-' . $request->bundleDiscount . ' ' . trans('webMessage.kd') . '</font></td>
									</tr>';
			}

			if (!empty(Cookie::get('gb_seller_discount'))) {
				$deliverytxt .= '<tr><th>' . strtoupper(trans('webMessage.seller_discount')) . '</th><td><font color="#FF0000">-' . trans('webMessage.kd') . '' . Cookie::get('gb_seller_discount') . '</font></td>
									</tr>';
	...
	*/
	...
	public static function getTagsImage($tags)
	{
		...
		if (!empty($tagsInfo->image)) {
			$tags = '<a href="' . url(app()->getLocale() . '/product-tag/' . $tags) . '"><img style="width:20px;height:20px;" width="20" height="20" class="tagimg" id="' . $tags . '" src="' . url('uploads/product/' . $tagsInfo->image) . '"></a>';
		} else {
	...
	public function cs_response_accept(Request $request)
	{
		...
		if (!empty($request->req_transaction_uuid)) {
			...
			<tr>
			  <td>' . trans('webMessage.amount') . '</td>
			  <td>' . number_format($amount, 3) . ' ' . trans('webMessage.kd') . '</td>
			</tr>
		  </table>';
				$trackYourOrderTxt = trans('webMessage.trackyourorderhistory') . '<br>' . url(app()->getLocale() . '/order-details') . '/' . md5($orderDetails->order_id);
				//send email to admins
	...
	public function ajaxAddProductToOrder(Request $request)
	{
		...
		if ($optionSc != 0) {
			if ($size_id != 0 && $color_id != 0) {
				$product_attribute = ProductAttribute::where('product_id', $request->product_id)->where('size_id', $size_id)->where('color_id', $color_id)->first();
			} elseif ($size_id != 0 && $color_id == 0) {
				$product_attribute = ProductAttribute::where('product_id', $request->product_id)->where('size_id', $size_id)->where('color_id', 0)->first();
			} elseif ($size_id == 0 && $color_id != 0) {
				$product_attribute = ProductAttribute::where('product_id', $request->product_id)->where('size_id', 0)->where('color_id', $color_id)->first();
			} else {
				$product_attribute = ProductAttribute::where('product_id', $request->product_id)->where('size_id', 0)->where('color_id', 0)->first();
			}
	...
	public function vendororderprint(Request $request)
	{
		...
		$orderDetailsTxt .= $ordertxt_child;
		$orderDetailsTxt .= '<tr><td colspan="4" align="right"><b>' . trans('webMessage.total') . '&nbsp;:&nbsp;&nbsp;</b></td><td>' . trans('webMessage.kd') . '' . $totalprice . '</td></tr>';
	...
	public function knet_response_q8link_return(Request $request)
	{
		if ($request->refid) {
			...
			$orderDetails = OrdersDetails::where('order_id', $refid)->first();
			if ($orderDetails->id) {
	    ...
	    <tr>
	      <td>' . trans('webMessage.amount') . '</td>
	      <td>' . number_format($jsdecode['udf2'], 3) . ' ' . trans('webMessage.kd') . '</td>
        </tr>';
